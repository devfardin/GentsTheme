...
<li <?php wc_product_class('gt-product-card', $product); ?> data-product_id="<?php echo esc_attr($product_id); ?>">
    ...
    <div class="gt-card-media">
        ...
        <a href="<?php echo esc_url($permalink); ?>" class="gt-card-img-link" tabindex="-1">
            <?php
            echo $product->get_image('woocommerce_medium', ['class' => 'gt-card-img gt-card-img--primary']);
            $gallery_ids = $product->get_gallery_image_ids();
            if (!empty($gallery_ids)) {
                echo wp_get_attachment_image($gallery_ids[0], 'woocommerce_medium', false, ['class' => 'gt-card-img gt-card-img--hover']);
            }
            ?>
        </a>
        ...
        <?php if ($badge_label): ?>
            <span
                class="gt-card-badge <?php echo esc_attr($badge_class); ?>"><?php echo wp_kses_post($badge_label); ?></span>
        <?php endif; ?>
    ...
    <div class="gt-card-body">
        ...
        <?php else: ?>
            <div class="gt-card-rating">
                <?php for ($i = 1; $i <= 5; $i++): ?>
                    <svg viewBox="0 0 24 24" class="gt-star<?php echo ($i <= round(0)) ? ' gt-star--on' : ''; ?>">
                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01z" />
                    </svg>
                <?php endfor; ?>
                <span class="gt-card-rating-count">(<?php echo esc_html($rating_count); ?>)</span>
